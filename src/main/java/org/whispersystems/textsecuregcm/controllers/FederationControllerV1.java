...
public class FederationControllerV1 extends FederationController {
      ...
      throws IOException
  {
    try {
      ...
      messageController.sendMessage(new NonLimitedAccount(source, sourceDeviceId, peer.getName()), authorizationHeader, destination, messages);
                                      ...
                                      @PathParam("offset") int offset)
  {
    ...
    for (Account account : accountList) {
      ...
      ClientContact clientContact = new ClientContact(token, account.getAuthToken(), null, account.isVoiceSupported(), account.isVideoSupported());
