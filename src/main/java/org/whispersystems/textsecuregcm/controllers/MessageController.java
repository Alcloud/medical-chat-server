...
import org.whispersystems.textsecuregcm.auth.AuthenticationHelper;
...
public class MessageController {
    ...
    public SendMessageResponse sendMessage(@Auth Account source,
        ...
        logger.debug("messages: " + messages.getMessages().toString());
        ...
        try {
            header = AuthorizationHeader.fromFullHeader(authorizationHeader);
        ...
        try {
            ...
            Account currentAccount = getDestinationAccount(source.getNumber());
            System.out.println("currentAccount credential: " + currentAccount.getNumber() + " / " + currentAccount.getAuthToken());
            System.out.println("credential from authorizationHeader: " + header.getPassword());

            // Check the token and send the message after check
            AuthenticationHelper authenticationHelper = new AuthenticationHelper();
            if(authenticationHelper.tokenRegistration(header.getPassword(), header.getNumber())){
            ...
            /*if (!currentAccount.getAuthToken().isEmpty()) {
                //TODO: check if Token abgelaufen ist, only then:
                // Soll man auch header.getPassword() und currentAccount.getAuthToken() vergleichen?
                if (Util.isEmpty(messages.getRelay()))
                    sendLocalMessage(source, destinationName, messages, isSyncMessage);
                else sendRelayMessage(source, destinationName, messages, isSyncMessage);
            } else {
                //TODO: access to IdentityProvider to get a new Token (or validate old one?)
                if(header.getPassword() != null) {
                    // TODO: validate header.getPassword()
                    source.setAuthToken(header.getPassword());
                    accountsManager.update(source);
                }
            }*/
    ...
    public OutgoingMessageEntityList getPendingMessages(@Auth Account account) {
        ...
        if (!Util.isEmpty(account.getAuthenticatedDevice().get().getApnId()) && apnFallbackManager != null) {
